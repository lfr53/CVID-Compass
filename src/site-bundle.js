...
const { addPageArt } = require("src/page-art.js");
const { stylePage } = require("src/page-layouts.js");
const { contributionView, homepage, patientEntry, localizeNavigation } = require("src/final-homepage.js");
...
function shell(title,intro,body){return `<div class="page-shell"><h1>${title}</h1><p class="section-intro">${intro}</p>${body}</div>`}
...
function profile(){let selected=Object.values(state.answers).flat();let diagnosis=selected.find(x=>['cvid','antibody_deficiency','suspected_iei','undiagnosed'].includes(x))||'undiagnosed';let feats=selected.filter(x=>labels[x]&&![diagnosis].includes(x));let matched=demoCases.filter(c=>c.diagnosis===diagnosis||feats.some(f=>c.features.includes(f)));let related=research.filter(r=>r.diagnoses.includes(diagnosis)||feats.some(f=>r.features?.includes(f)));return shell('Your case profile','Here is a transparent view of the features you chose — not a diagnosis or a medical match score.',`<div class="panel"><div>${tag(diagnosis)}${chips(feats)}</div><p class="fine-print">Shared features help you explore patterns. They do not mean the same diagnosis or clinical equivalence.</p></div><section class="section"><h2>Cases with shared features</h2><p class="section-intro">${matched.length?matched.length+' anonymous demo cases share at least one of these features.':'No closely related network cases have been contributed yet.'}</p><div class="card-grid">${matched.slice(0,3).map((c,i)=>`<article class="researcher-card"><b>Anonymous phenotype case ${i+1}</b><p>${chips([c.diagnosis,...c.features,...c.immune,(c.genetic==='negative'?'negative':'unknown')])}</p><span class="fine-print">Shared tags only · no identity exposed</span></article>`).join('')}</div></section><section class="section"><h2>Research connected to these features</h2><div class="card-grid">${related.map(card).join('')}</div></section><section class="section"><h2>People studying these features</h2><div class="card-grid">${researchers.slice(0,3).map(r=>`<article class="researcher-card"><h3>${r[1]}</h3><p>${r[2]}</p><div>${r[3]}</div></article>`).join('')}</div></section><section class="section"><h2>Communities for people like me</h2><div class="panel"><h3>Patient organisations and IEI communities</h3><p>Explore CVID and broader inborn-errors-of-immunity communities through trusted organisations such as Immunodeficiency UK, IPOPI and the Immune Deficiency Foundation.</p><a href="#community">Explore communities →</a></div></section><section class="section"><h2>Related CVID education</h2><a class="button secondary" href="#cvid">Explore topics →</a><div class="article-bridge"><h2>Keep up with CVID research</h2><p>Browse research and resources as you return. Email subscriptions are not available in this version.</p><a href="#research">Explore research →</a></div><div class="article-bridge"><h2>Help build a clearer picture of CVID</h2><p>By contributing an anonymous case profile, you can help patients, clinicians and researchers recognise shared patterns.</p><a href="#contribute">Add my anonymous case →</a></div></section>`)}
...
function render(){let h=location.hash.slice(1)||'home';let html=h==='home'?homepage(document.documentElement.lang):h.startsWith('article-')?educationView(document.documentElement.lang,h.slice(8)):(h.startsWith('cvid/topic/')||h.startsWith('cvid/paper/'))?educationView(document.documentElement.lang,h.slice(5)):h==='cvid'?educationView(document.documentElement.lang):h==='contribute'?contributionView(document.documentElement.lang):h==='patients'?patientEntry(document.documentElement.lang):h==='find'?patientForm(q,labels,state.answers,document.documentElement.lang):h==='find/results'?profile():h==='research'?researchPage():h==='clinicians'?exploration('clinician',demoCases,research,labels,document.documentElement.lang):h==='researchers/explore'?exploration('researcher',demoCases,research,labels,document.documentElement.lang):h==='researchers'?directory(document.documentElement.lang,'researchers',researchers):h==='community'?directory(document.documentElement.lang,'community'):h.startsWith('research/')?shell('Research detail','A public research record connected to phenotype tags and its source.',card(research.find(r=>r.id===h.split('/')[1])||research[0])):homepage(document.documentElement.lang);document.querySelector('#app').innerHTML=html;document.querySelectorAll('[data-option]').forEach(b=>b.onclick=()=>{let v=b.dataset.option;state.answers[state.step]=state.answers[state.step]||[];state.answers[state.step]=state.answers[state.step].includes(v)?state.answers[state.step].filter(x=>x!==v):[...state.answers[state.step],v];render()});document.querySelector('#prev')?.addEventListener('click',()=>{state.step--;render()});document.querySelector('#next')?.addEventListener('click',()=>{if(state.step<q.length-1){state.step++;render()}else{location.hash='find/results'}});if(h==='clinicians'||h==='researchers/explore')bindExploration(h==='clinicians'?'clinician':'researcher',render);stylePage(h,document.documentElement.lang);addPageArt(h);bindPatientForm(state);bindDirectory();localizeNavigation(document.documentElement.lang);const toggle=document.querySelector('#language-toggle');if(toggle){toggle.textContent=document.documentElement.lang==='zh'?'English':'中文';toggle.onclick=()=>{document.documentElement.lang=document.documentElement.lang==='zh'?'en':'zh';render()}}}
research.push(...extraProjects());
window.addEventListener('hashchange',render);render();

Object.assign(exports, {});
},
"src/page-art.js": function(require, exports) {
function addPageArt(route){
...
Object.assign(exports, {addPageArt});
},
"src/page-layouts.js": function(require, exports) {
const { symbol } = require("src/reference-visuals.js");
function stylePage(route,lang){
...
function homepage(lang='en'){
 ...
 const oldAngles=[['Cases with shared features','具有共同特征的病例','Explore published cases, case series, cohorts and anonymous network cases with related clinical or immune features.','探索具有相关临床或免疫特征的已发表病例、病例系列、队列和匿名网络病例。','Explore cases','探索病例','#patients'],['Research connected to these features','与这些特征相关的研究','Find natural-history studies, cohorts, genetics projects, registries and clinical trials involving similar patient groups.','查找涉及相似患者群体的自然病程研究、队列、遗传学项目、登记系统和临床试验。','Explore research','探索研究','#research'],['People studying this','研究这些问题的人','Find clinicians, researchers and specialist centres working with related CVID phenotypes.','寻找关注相关 CVID 表型的临床医生、研究者与专科中心。','Explore specialists','探索专业团队','#researchers'],['Communities for people like me','与我相关的社群','Find patient organisations and communities connected to CVID and related immune conditions.','寻找与 CVID 和相关免疫疾病有关的患者组织及社群。','Explore communities','探索社群','#community']];
 const angles=[['Learn about CVID','了解 CVID','Clear, patient-friendly information on symptoms, diagnosis, treatment, complications, genetics and research.','以易懂的方式了解症状、诊断、治疗、并发症、遗传学与研究。','Explore topics','浏览主题','#cvid'],['Explore cases','探索病例','Browse published cases, case series and cohorts with related features.','探索具有相关特征的已发表病例、病例系列和队列。','Explore cases','探索病例','#clinicians'],['Discover research','发现研究','Find studies, cohorts, registries, clinical trials and research programmes.','了解研究、队列、登记系统、临床试验及研究项目。','Explore research','探索研究','#research'],['Find communities','寻找社群','Patient organisations, communities and support resources.','寻找患者组织、社群与支持资源。','Explore communities','探索社群','#community']];
 return `<div class="reference-hero-wrap"><section class="hero discovery-hero"><div><div class="eyebrow">${t('A more connected view of CVID','以更多连接了解 CVID')}</div><h1>${t('Connecting the CVID<br class="hero-title-break">community through<br class="hero-title-break">shared patterns','以共同的疾病特征，<span>连接 CVID 社群</span>')}</h1><p class="hero-promise">${t('Reliable information. Real experiences. A stronger community.','可靠的信息，真实的经历，更紧密的社群。')}</p><p>${t('CVID Compass brings together patients and families, clinicians, and researchers to share knowledge, explore patterns and accelerate research.','围绕共同的临床特征、免疫表现和研究，连接患者与家属、临床医生及研究者。')}</p><div class="hero-actions compact-role-actions">${roles.map(([en,zh,url],i)=>`<a class="button ${i?'secondary':'primary'}" href="${url}">${t(en,zh)}</a>`).join('')}</div><p class="hero-support">${t('Explore CVID through what is already known — from published cases and research cohorts to specialist centres, ongoing studies and, over time, anonymous cases contributed to the network.','从已有知识出发探索 CVID：包括已发表病例、研究队列、专科中心、正在进行的研究，以及未来逐渐汇入网络的匿名病例。')}</p><div class="prominent-roles hero-left-roles"><div class="role-grid">${roles.map(([en,zh,url],i)=>`<a class="role-entry" href="${url}">${symbol(['people','clinician','flask'][i])}<h2>${t(en,zh)}</h2><p>${t(['Understand & explore CVID','Explore cases & resources','Find data & collaborators'][i],['了解并探索我的情况','探索病例与相关专业团队','探索病例或研究表型'][i])}</p><span>${t('Start exploring','开始探索')} →</span></a>`).join('')}</div></div></div>${referenceArtwork(lang)}</section></div><section class="section home-exploration"><div class="eyebrow">${t('Explore CVID from different angles','从不同角度探索 CVID')}</div><h2>${t('Information, support and opportunity','发现与你相关的内容')}</h2><div class="angle-grid">${angles.map(([en,zh,body,bodyZh,cta,ctaZh,url],i)=>`<article class="panel"><div class="angle-heading">${symbol(['book','document','chart','people'][i])}<h3>${t(en,zh)}</h3></div><p>${t(body,bodyZh)}</p><a class="back-link" href="${url}">${t(cta,ctaZh)} →</a></article>`).join('')}</div></section><section class="section contribution-section"><div class="contribution-band">${symbol('heart')}<div><h2>${t('Help build a clearer picture of CVID','共同建立更清晰的 CVID 图景')}</h2><p>${t('By sharing an anonymous case profile, you can help connect patterns and support future research.','分享匿名病例档案，帮助连接疾病特征，为未来研究提供支持。')}</p></div><a class="button primary" href="#contribute">${t('Add my anonymous case','添加我的匿名病例')} →</a></div></section>`;
}
function patientEntry(lang){const t=(en,zh)=>w(lang,en,zh);return `<section class="page-shell patient-landing"><svg class="patient-leaves" viewBox="0 0 300 240" aria-hidden="true"><ellipse cx="155" cy="120" rx="145" ry="110" fill="#edf4ef"/><g fill="#c5ded3" fill-opacity=".7"><path d="M120 185C50 170 42 125 69 111c36 6 49 39 51 74Z"/><path d="M129 154C99 113 104 68 130 62c28 22 22 57-1 92Z"/><path d="M143 143c13-47 49-73 80-57-1 36-34 58-80 57Z"/><path d="M164 101c-20-36-9-68 15-82 18 22 13 56-15 82Z"/><path d="M181 85c4-37 39-62 72-68 4 38-28 68-72 68Z"/></g><g stroke="#91b9a8" stroke-width="2" fill="none"><path d="M99 222Q126 143 234 40M123 167 78 129M139 141 132 82M154 121 204 103M173 93 179 39"/></g></svg><div class="eyebrow">${t('For patients & families','面向患者与家属')}</div><h1>${t('Living with or learning about CVID?','正在与 CVID 共处，或想了解 CVID？')}</h1><p class="article-lead">${t('Find clear information about CVID, understand different symptoms and complications, and explore what research may be relevant to your situation.','了解 CVID、症状与并发症，探索与你的情况相关的信息、社群和研究。')}</p><div class="patient-paths">${[['Understand CVID','了解 CVID','Understand symptoms, diagnosis, treatment and more.','了解症状、诊断、治疗等知识。','#cvid','book'],['Explore my situation','探索我的情况','Find information and research related to what you already know.','从已知信息出发，探索相关资料和研究。','#find','search'],['Latest CVID research','CVID 研究进展','Explore research programmes, studies and new resources.','探索研究项目、临床研究与相关资源。','#research','chart']].map(([en,zh,body,cn,url,icon])=>`<a class="panel reading-card" href="${url}">${symbol(icon)}<h2>${t(en,zh)}</h2><p>${t(body,cn)}</p><span>${t('Explore','开始了解')} →</span></a>`).join('')}</div><h2>${t('What would you like to understand?','你想了解什么？')}</h2><nav class="topic-links" aria-label="CVID topics">${[['What is CVID?','什么是 CVID？','cvid/topic/basics'],['Symptoms & diagnosis','症状与诊断','article-lab-results'],['Why is CVID different for everyone?','为什么每个人的 CVID 都不同？','article-multi-organ'],['Complications','并发症','article-multi-organ'],['Treatment','治疗','article-immunoglobulin-replacement'],['Genetics','遗传学','cvid/topic/genetics'],['Research & clinical studies','研究与临床研究','cvid/topic/research']].map(([en,zh,url])=>`<a href="#${url}">${t(en,zh)}</a>`).join('')}</nav><a class="back-link" href="#community">${t('Find communities and support','寻找社群与支持')} →</a></section>`}
function contributionView(lang){const t=(en,zh)=>w(lang,en,zh);return `<section class="page-shell"><h1>${t('Help build a clearer picture of CVID','共同建立更清晰的 CVID 图景')}</h1><div class="panel"><h2>${t('Anonymous contributions are coming soon','匿名病例贡献功能即将开放')}</h2><p>${t('This version does not collect or publish case profiles. You can explore your situation now; your answers remain in this page and are not submitted.','当前版本不收集或发布病例档案。你可以先探索自己的情况；填写的信息仅保留在此页面中，不会提交。')}</p><a class="button primary" href="#find">${t('Explore my situation','探索我的情况')} →</a></div></section>`}
function localizeNavigation(lang){const t=(en,zh)=>w(lang,en,zh);document.querySelector('.site-header nav').innerHTML=[['CVID','了解 CVID','#cvid'],['Explore Cases','探索病例','#clinicians'],['Research','研究','#research'],['Centers','研究中心','#researchers'],['Community','社群','#community']].map(([en,zh,url])=>`<a href="${url}">${t(en,zh)}</a>`).join('');let cta=document.querySelector('#header-explore');if(!cta){cta=document.createElement('a');cta.id='header-explore';cta.className='button primary';cta.href='#patients';document.querySelector('.site-header').insertBefore(cta,document.querySelector('#language-toggle'));}cta.textContent=t('Understand & explore CVID','了解与探索 CVID');}

Object.assign(exports, {homepage,patientEntry,contributionView,localizeNavigation});
},
"src/patient-form.js": function(require, exports) {
function patientForm(q,labels,answers,lang){const t=(en,zh)=>lang==='zh'?zh:en;const titles=['Diagnosis','Clinical features','Immunoglobulins','Vaccine response','Immune-cell testing','Genetics','Family history','Treatment history','Age / onset'];const zhTitles=['诊断','临床表现','免疫球蛋白','疫苗应答','免疫细胞检测','遗传学','家族史','治疗史','年龄 / 起病阶段'];return `<div class="page-shell"><h1>${t('Explore my case','探索我的病例')}</h1><p>${t('Add what you already know. Leave anything you are unsure about blank.','填写你已经知道的信息。不确定的项目可以留空。')}</p><div class="patient-form-grid">${q.map(([question,options],i)=>`<fieldset class="panel"><legend>${lang==='zh'?zhTitles[i]:titles[i]}</legend><div class="option-grid">${options.map(o=>`<label class="option"><input type="${[1,2,7].includes(i)?'checkbox':'radio'}" name="dimension-${i}" data-question="${i}" value="${o}" ${answers[i]?.includes(o)?'checked':''}> ${labels[o]||o.replaceAll('_',' ')}</label>`).join('')}</div></fieldset>`).join('')}</div><div class="hero-actions"><button class="primary" id="explore-profile">${t('Explore my profile','探索我的档案')}</button><button type="button" class="secondary" id="clear-profile">${t('Clear form','清空表单')}</button></div></div>`;}
function bindPatientForm(state){document.querySelectorAll('[data-question]').forEach(input=>input.onchange=()=>{const i=Number(input.dataset.question);state.answers[i]=[...document.querySelectorAll(`[data-question="${i}"]:checked`)].map(el=>el.value)});const clear=document.querySelector('#clear-profile');if(clear)clear.onclick=()=>{state.answers={};document.querySelectorAll('[data-question]').forEach(input=>input.checked=false)};const submit=document.querySelector('#explore-profile');if(submit)submit.onclick=()=>location.hash='find/results';}
...
function exploration(mode, cases, projects, labels, lang) {
  ...
  return `<div class="page-shell"><h1>${mode==='clinician'?t('Explore similar cases','探索相似病例'):t('Explore patient groups','探索患者群体')}</h1><p class="section-intro">${mode==='clinician'?t('Have you seen a case like mine? Start with the findings in one case.','有没有人见过类似的病例？从一个病例的检测结果和临床表现开始。'):t('Where are the patients with the phenotype I want to study? Define the group below.','我想研究的表型对应哪些患者？在下方定义你关注的群体。')}</p>
  ...
  <p class="demo-notice">${t('No community cases have been contributed to this version yet. Explore existing research and centres below.','此版本尚未收录社群贡献的病例。可继续探索下方的现有研究与中心。')}</p>
  ...
  ${!found.length?`<div class="empty">${t('No closely related network cases have been contributed yet. Related research and centres are listed below.','暂未有包含这一组合的匿名病例。下方仍可探索相关研究和中心。')}</div>`:''}
...
const { topicIntro, readingHighlights, nextReading } = require("src/reading-support.js");
const { learningHub } = require("src/learning-hub.js");
...
Object.assign(exports, {topicIntro,readingHighlights,nextReading});
},
"src/learning-hub.js": function(require, exports) {
const drawings={person:'<circle cx="50" cy="30" r="14"/><path d="M24 86V68a26 26 0 0 1 52 0v18Z"/>',lungs:'<path d="M47 12v37L30 66M53 12v37l17 17M43 37C24 30 11 56 13 81c18 8 32-5 30-22ZM57 37c19-7 32 19 30 44-18 8-32-5-30-22Z"/>',tests:'<path d="M18 19h20v59a10 10 0 0 1-20 0ZM14 12h28v10H14ZM22 48h12M53 24h32M53 39h24M53 54h29M55 85V70h6v15M68 85V61h6v24M81 85V51h6v34"/>',dna:'<path d="M29 12c0 40 42 36 42 76M71 12c0 40-42 36-42 76M30 20h40M34 33h32M35 66h30M30 80h40M44 46h12M44 54h12"/>',family:'<circle cx="35" cy="28" r="13"/><circle cx="68" cy="39" r="11"/><path d="M12 86V67a23 23 0 0 1 46 0v19M57 59a20 20 0 0 1 31 16v11H58"/>',scope:'<path d="m44 10 18 9-19 38-18-9ZM31 53l-5 10 15 7 5-10M58 37a25 25 0 0 1 1 49H28M24 88h56M29 75h27"/>'};
function learningHub(lang){const zh=lang==='zh',t=(en,cn)=>zh?cn:en;
 const topics=[['person','What is CVID?','什么是 CVID？','An overview of CVID, the immune system and why it is different for everyone.','了解 CVID、免疫系统，以及为什么每个人的表现不同。','infection'],['lungs','Symptoms and complications','症状与并发症','Common symptoms and the wider health effects of CVID.','了解 CVID 常见症状及对不同器官的影响。','multi-organ'],['tests','Immune tests & treatment','免疫检测和治疗','Understand immune test results and immunoglobulin replacement therapy.','读懂免疫检测报告，了解免疫球蛋白替代治疗。','lab-results'],['dna','Genetics','遗传学','What we know about genetics in CVID and when to consider genetic testing.','了解 CVID 遗传学与遗传检测。','genetics'],['family','Family','家庭','Information for family members, inheritance and practical considerations.','面向家属的信息、遗传问题与日常考虑。','family'],['scope','Research and clinical studies','研究与临床研究','Ongoing research, clinical trials and how you can get involved.','了解研究、临床研究与参与方式。','research-progress']];
 return `<section class="learning-hub"><header><div class="eyebrow">${t('Learn about CVID','了解 CVID')}</div><h1>${t('Understand CVID, step by step','一步一步，了解 CVID')}</h1><p>${t('Clear, practical and evidence-informed information to help you understand CVID — from the basics to diagnosis, treatment, genetics, family and research.','清晰、实用、有依据的信息，帮助你从基础知识逐步了解诊断、治疗、遗传学、家庭与研究。')}</p></header><div class="learning-topic-grid">${topics.map(([icon,en,cn,body,bodyZh,slug])=>`<a class="learning-topic" href="${slug==='lab-results'?'#cvid/topic/tests-treatment':slug==='genetics'?'#cvid/topic/genetics':slug==='research-progress'?'#cvid/topic/research':'#article-'+slug}"><svg class="topic-illustration" viewBox="0 0 100 100" aria-hidden="true"><circle class="icon-halo" cx="50" cy="50" r="48"/><g>${drawings[icon]}</g></svg><h2>${t(en,cn)}</h2><p>${t(body,bodyZh)}</p><span class="topic-arrow" aria-hidden="true">→</span></a>`).join('')}</div></section>`;
}

Object.assign(exports, {learningHub});
},
